...
PTSolns_I2Connect_Digit_Display::PTSolns_I2Connect_Digit_Display() {
  _i2c_addr = 0x70; 
}

bool PTSolns_I2Connect_Digit_Display::begin(uint8_t address, TwoWire &wirePort) {
  _i2c_addr = address;
  _wire = &wirePort;
...
void PTSolns_I2Connect_Digit_Display::initHT16K33() {
  _wire->beginTransmission(_i2c_addr);
...
void PTSolns_I2Connect_Digit_Display::setBrightness(uint8_t b) {
  if (b > 15) b = 15;
  _wire->beginTransmission(_i2c_addr);
...
void PTSolns_I2Connect_Digit_Display::blinkRate(uint8_t b) {
  if (b > 3) b = 0; 
  _wire->beginTransmission(_i2c_addr);
...
void PTSolns_I2Connect_Digit_Display::writeDigitRaw(uint8_t digit, uint8_t bitmask) {
  if (digit > 7) return;
  displayBuffer[digit] = bitmask;
}

void PTSolns_I2Connect_Digit_Display::clear() {
  for (uint8_t i = 0; i < 8; i++) {
    displayBuffer[i] = 0;
  }
}

void PTSolns_I2Connect_Digit_Display::writeDisplay() {
  _wire->beginTransmission(_i2c_addr);
...
uint8_t PTSolns_I2Connect_Digit_Display::encodeDigit(uint8_t digit) {
  if (digit > 9) return 0x00; 
  return numberFont[digit];
}

void PTSolns_I2Connect_Digit_Display::printLeft(int16_t number, bool leftJustify) {
...
void PTSolns_I2Connect_Digit_Display::printRight(int16_t number, bool leftJustify) {
...
void PTSolns_I2Connect_Digit_Display::print8(int32_t number, bool leftJustify) {
  clear();
  String numStr = String(number);
  if(numStr.length() > 8) {
    printLeft(9999); 
